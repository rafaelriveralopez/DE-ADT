 ...
 * @param <S>
 */
public class DEADTEvaluator<S extends Solution<?>> extends SequentialSolutionListEvaluator<S> {

  @Override
  public List<S> evaluate(List<S> solutionList, Problem<S> problem) throws JMetalException {
    try {
      if(problem instanceof DEADTProblem){
        for (int i = 0 ; i < solutionList.size(); i++) {
          ((DEADTProblem) problem).setInd(i);
          problem.evaluate(solutionList.get(i));
        }     
      } else if (problem instanceof ConstrainedProblem) {
        for (int i = 0 ; i < solutionList.size(); i++) {
          problem.evaluate(solutionList.get(i));
